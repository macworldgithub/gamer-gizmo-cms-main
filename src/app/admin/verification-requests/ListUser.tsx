...
const ListUser = () => {
  ...
  //     setAdminToken(localStorage.getItem("admin-x-token"));
  //   }
  // }, []);
  ...
  const fetchUsers = async (page: number, pageSize: number) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/getVerificationRequests?pageNo=${page}&pageSize=${pageSize}`,
  ...
  const handleApproval = async (id: number) => {
    try {
      await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/approveUserVerification?userId=${id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      render: (_: any, user: User) => (
        <div className="flex gap-2">
          ...
          <Popconfirm
            ...
            onConfirm={() => handleApproval(user.id)}
            okText="Yes"
            cancelText="No"
          >
            <Button
