...
const ListUser = () => {
  ...
  const fetchUsers = async (page: number, pageSize: number) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/getAllUsers?pageNo=${page}&pageSize=${pageSize}`,
  ...
  const handleDeactivateUser = async (id: number) => {
    try {
      await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/changeUserStatus?userId=${id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchUsers(currentPage, pageSize);
      toast.success("User Status Chnaged successfully");
      fetchUsers(currentPage, pageSize);
    } catch {
  ...
  const handleDeleteUser = async (id: number) => {
    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/deleteUser?userId=${id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
